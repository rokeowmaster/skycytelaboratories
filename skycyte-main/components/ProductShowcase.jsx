...
const ProductShowcase = () => {
  return (
    <div className='bg-gray-100 backdrop-blur-lg bg-opacity-50'>
      ...
      <section className="w-full max-w-5xl py-16 mx-auto text-center">
        <h2 className="text-3xl mb-6 font-bold text-blue-600">Featured Products</h2>
        <div className="flex justify-center">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-4">
            {products.map(({ id, name, desc, image }) => (
              <Card key={id} className="p-6 transform hover:scale-105 transition">
                <CardContent className="text-center">
                  <Image src={image} alt={name} width={300} height={160} className="w-full h-40 object-cover mb-3 rounded-lg" />
